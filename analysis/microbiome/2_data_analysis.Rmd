...
```{r}
set.seed(3)
cols = turbo(16)[sample(1:16,16)]

aldex_full |> 
  filter(diff.btw > (diff.btw/diff.win)) |> 
  ...
  summarise(mean_diff=mean(diff.btw), 
            ...
            total=n())|> 
  ggplot() +
  geom_point(aes(color = Phylum, y = mean_disp, 
                 x = mean_diff, size = total)
             ) +
  geom_text_repel(aes(x = mean_diff, y = mean_disp, 
                label = Class)) +
